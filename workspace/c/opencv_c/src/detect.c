...
void initRecorder() {
	...
	s[strlen(s) - 2] = 0;
	s[strlen(s) - 1] = 0;
	writer = cvCreateVideoWriter(s, CV_FOURCC('M', 'J', 'P', 'G'), 25, cvGetSize(img), 1);
	//printf("Font = %i", font);
	...
	while (1) {
		...
		if (!recording) {
			...
			if (now >= (detection_time + 6)) {
				...
				//printf("Putting ");
				cvPutText(currentImg, "Hello this is me", cvPoint(25, 0), &font, CV_RGB(0,0,0));
